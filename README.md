...
```
版本1
1.sms短信数据库已经全部弄完了，后面要思考下个人中心登陆的功能，是用什么来搞定
2.杯具的事：昨天弄的数据库的数据一不小心没备份被删掉了，重来了一遍
```

### 2014年5月13日
```
版本1
1.程序方面没大改进，但是开始在研究android网络编程这块内容，因为项目中有不少可以用，所以就直接在这个项目中开了java文件
  在写代码，后面要改进从而能登录服务器，这方面的东西还在构建中
```

### 2014年5月16日
```
版本1
1.这几天学习了下android的网络编程，并且用php在sae上面搭建了简单的服务器，然后可以通过GET POST方式进行一些操作了
2.用PHP实现了简单的网页抓取功能，得到一些数据
3.思考了下birthday应用的登陆功能的制作方式
4.大概思考了下注册最终准备通过md5或其他不可逆算法来实现，可通过POST或者GET方式发送到sae的php上
5.完成了登陆和注册的界面

版本2
1.适应SAE PHP的使用，并编写了服务器端的代码
2.修改birthday这边项目的程序，最终完成登陆功能
```

### 2014年5月17日
```
版本1
1.SAE PHP端修改了注册和登陆的代码
2.android这边完成了注册和登陆功能
```

### 2014年5月19日
```
版本1
1.开始工作，每天加班到8点半，但是这个项目会做到完整，感觉还有完善的地方，后续会加上更新等功能
2.刚刚完成了注册登陆界面这边的一些逻辑处理。
```

### 2014年5月20日
```
版本1
1.解决了center界面中的基本大部分功能，后续把备份功能加上，还有百度百科这种准备通过网页爬虫搞定
2.努力完成一个完整的项目
```